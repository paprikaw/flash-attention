...
////////////////////////////////////////////////////////////////////////////////////////////////////
// Optimized version: resolve both K and V addresses with single block_table lookup
// This reduces memory accesses by sharing the page_idx lookup between K and V
template <typename Kernel_traits>
__forceinline__ __device__
void flexi_resolve_thread_kv_pair_offset(
    const int tidx, const int n_block, const int page_block_size, 
    ...
    using Element = typename Kernel_traits::Element;
    // Single block_table lookup - shared between K and V
    const int page_idx = __ldg(block_table + virtual_page_idx);
    
    // Load K and V base addresses
    const uintptr_t k_base_addr = __ldg(reinterpret_cast<const uintptr_t*>(k_page_ptrs) + page_idx);
    const uintptr_t v_base_addr = __ldg(reinterpret_cast<const uintptr_t*>(v_page_ptrs) + page_idx);

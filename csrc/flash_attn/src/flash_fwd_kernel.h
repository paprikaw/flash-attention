...
inline __device__ void compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    uint64_t resolve_cycles_all = 0;  // Per-thread accumulator for resolve cycles
    uint64_t resolve_cycles_t0 = 0;   // Thread 0 accumulator for resolve cycles
#endif    
    ...
    constexpr int kHeadDim = Kernel_traits::kHeadDim;
    constexpr int kNWarps = Kernel_traits::kNWarps;
    // 定义从share memory中拷贝数据的strategy（全局/累加）
...
#ifdef DEBUG_FLEXI_TIMING
    now = clock64();
    resolve_cycles_all += (now - before);
    if (tidx == 0) {
        resolve_cycles_t0 += (now - before);
    }
#endif
    ...
    // All threads statistics
    if (params.debug_timing != nullptr) {
        uint64_t t_end = clock64();
        atomicAdd(reinterpret_cast<unsigned long long*>(params.debug_timing + 3),
                  static_cast<unsigned long long>(resolve_cycles_all));
        atomicAdd(reinterpret_cast<unsigned long long*>(params.debug_timing + 4),
                  static_cast<unsigned long long>(t_end - t_start));
        atomicAdd(reinterpret_cast<unsigned long long*>(params.debug_timing + 5),
                  static_cast<unsigned long long>(1));
    }
#endif
}
...
template<typename Kernel_traits, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Is_softcap, bool Split, bool Append_KV, typename Params>
inline __device__ void flexi_compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    using ElementAccum = typename Kernel_traits::ElementAccum;
    using index_t = typename Kernel_traits::index_t;
    assert (params.k_page_ptrs != nullptr && params.v_page_ptrs != nullptr);
    // Shared memory.
    extern __shared__ char smem_[];
    ...
    if (block_table != nullptr) {
        auto final_block_size = binfo.actual_seqlen_k - (n_block_max - 1) * kBlockN;
        // Traditional mode: use block_table to lookup page_idx first
        tKgK.data() = flash::flexi_resolve_thread_kv_page_slice_offset<Kernel_traits>(tidx, n_block_max - 1, params.page_block_size,
            block_table, params.k_batch_stride, params.k_row_stride, params.k_page_ptrs, final_block_size) + head_offset_k;
...
template<typename Kernel_traits, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Is_softcap, bool Split, bool Append_KV, typename Params>
inline __device__ void flexi_direct_compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    using ElementAccum = typename Kernel_traits::ElementAccum;
    using index_t = typename Kernel_traits::index_t;
    // For direct pointer table mode, we use k_ptr_table and v_ptr_table instead of k_page_ptrs/v_page_ptrs
    assert (params.k_ptr_table != nullptr && params.v_ptr_table != nullptr);
    assert (params.use_direct_ptr_table == true);
    extern __shared__ char smem_[];
    ...
    // We move K and V to the last block.
    const int bidb_cache = params.cache_batch_idx == nullptr ? bidb : params.cache_batch_idx[bidb];
    const uintptr_t* k_ptr_table = params.k_ptr_table == nullptr ? nullptr : params.k_ptr_table + bidb * params.ptr_table_batch_stride;
    const uintptr_t* v_ptr_table = params.v_ptr_table == nullptr ? nullptr : params.v_ptr_table + bidb * params.ptr_table_batch_stride;
    const index_t head_offset_k = (bidh / params.h_h_k_ratio) * params.k_head_stride; // block addresses are later resolved per-thread
    const index_t head_offset_v = (bidh / params.h_h_k_ratio) * params.v_head_stride;
    // const index_t row_offset_k = block_table == nullptr
    //     ? binfo.k_offset(params.k_batch_stride, params.k_row_stride, bidb_cache)
    ...
    before = clock64();
#endif
    if (v_ptr_table != nullptr || k_ptr_table != nullptr) {
        const int final_block_size = binfo.actual_seqlen_k - (n_block_max - 1) * kBlockN;
            // Direct pointer table mode: resolve BOTH K and V pointers in a single call
            // This saves redundant computation of virtual_page_idx, page_offset, col_offset
            auto [k_resolved, v_resolved] = flash::flexi_direct_resolve_kv_pair_offset<Kernel_traits>(
                tidx, n_block_max - 1, params.page_block_size,
                k_ptr_table, v_ptr_table,
                params.k_row_stride, params.v_row_stride,
                final_block_size);
            tKgK.data() = k_resolved + head_offset_k;
            tVgV.data() = v_resolved + head_offset_v;
    }
#ifdef DEBUG_FLEXI_TIMING
    now = clock64();
    resolve_cycles_all += (now - before);
        ...
        for (int n_block = n_block_max - 1; n_block >= n_block_copy_min; n_block--) {
            ...
            if (v_ptr_table == nullptr && k_ptr_table == nullptr) {
                ...
                if (n_block > n_block_copy_min) {
                    ...
                    before = clock64();
#endif
                        // Optimized: resolve BOTH K and V in single call
                        auto [k_resolved, v_resolved] = flash::flexi_direct_resolve_kv_pair_offset<Kernel_traits>(
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        // Advance gV
        // NOTE: When masking_step > 0 and using ptr_table, V was already pre-resolved at the end
        // of the previous iteration (together with K), so we don't need to resolve V again.
        // Only update for non-paged case.
        if (masking_step > 0) {
            if (v_ptr_table == nullptr && k_ptr_table == nullptr) {
                tVgV.data() = tVgV.data() + (-int(kBlockN * params.v_row_stride));
            }
            // For ptr_table case, V was already pre-resolved in previous iteration's K resolve
            FLASH_NAMESPACE::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_KV, tVgV, tVsV, tKVcKV, tKVpKV);
        } else {
            // Clear the smem tiles to account for predicated off loads
            FLASH_NAMESPACE::copy<Is_even_MN, Is_even_K, /*Clear_OOB_MN=*/true>(
        ...
        if (n_block > n_block_min) {
            // Advance gK (and pre-resolve gV for next iteration)
            if (v_ptr_table == nullptr && k_ptr_table == nullptr) {
                tKgK.data() = tKgK.data() + (-int(kBlockN * params.k_row_stride));
                // Pre-advance V for next iteration too
                tVgV.data() = tVgV.data() + (-int(kBlockN * params.v_row_stride));
            } else {
#ifdef DEBUG_FLEXI_TIMING
                before = clock64();
#endif
                // OPTIMIZATION: resolve BOTH K and V for next n_block in single call
                // K[n_block-1] is for next iteration's GEMM (Q*K)
                // V[n_block-1] is for next iteration's copy (so we don't need to resolve V again)
                auto [k_resolved, v_resolved] = flash::flexi_direct_resolve_kv_pair_offset<Kernel_traits>(
                    tidx, n_block - 1, params.page_block_size,
                    k_ptr_table, v_ptr_table,
                    params.k_row_stride, params.v_row_stride);
    ...
    // Track whether this is the first iteration of non-masking loop
    // If V was pre-resolved in masking loop, this stays true. Otherwise we need to resolve V first.
    bool v_pre_resolved = (n_block < n_block_max - 1);  // Was there at least one pre-resolve in masking loop?
    ...
    for (; n_block >= n_block_min; --n_block) {
        ...
        if (v_ptr_table == nullptr && k_ptr_table == nullptr) {
            tVgV.data() = tVgV.data() + (-int(kBlockN * params.v_row_stride));
        } else if (!v_pre_resolved) {
            // First iteration: V was NOT pre-resolved, need to resolve it now
#ifdef DEBUG_FLEXI_TIMING
            before = clock64();
#endif
            tVgV.data() = flash::flexi_direct_resolve_thread_kv_pair_offset<Kernel_traits>(
                tidx, n_block, params.page_block_size,
                v_ptr_table, params.k_row_stride, params.v_row_stride) + head_offset_v;
#ifdef DEBUG_FLEXI_TIMING
            now = clock64();
            resolve_cycles_all += (now - before);
            if (tidx == 0) {
...
#endif
            v_pre_resolved = true;  // Mark as resolved for subsequent iterations
        }
        // For ptr_table case when v_pre_resolved=true, V was already pre-resolved
        ...
        if (n_block > n_block_min) {
            // Advance gK (and pre-resolve gV for next iteration)
            if (k_ptr_table == nullptr && v_ptr_table == nullptr) {
                tKgK.data() = tKgK.data() + (-int(kBlockN * params.k_row_stride));
                // Pre-advance V for next iteration too
                tVgV.data() = tVgV.data() + (-int(kBlockN * params.v_row_stride));
            } else {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
template<typename Kernel_traits, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Is_softcap, bool Split, bool Append_KV, typename Params>
inline __device__ void flexi_compute_attn_splitkv(const Params &params) {
    const int m_block = blockIdx.x;
    // The block index for the batch.
    const int bidb = Split ? blockIdx.z / params.h : blockIdx.y;
    // The block index for the head.
    ...
    const int n_split_idx = Split ? blockIdx.y : 0;
    const int num_n_splits = Split ? gridDim.y : 1;
    // Use compile-time constant for direct pointer table mode
    FLASH_NAMESPACE::flexi_compute_attn_1rowblock_splitkv<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV>(params, bidb, bidh, m_block, n_split_idx, num_n_splits);
}

////////////////////////////////////////////////////////////////////////////////////////////////////
template<typename Kernel_traits, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Is_softcap, bool Split, bool Append_KV, typename Params>
inline __device__ void flexi_direct_compute_attn_splitkv(const Params &params) {
    const int m_block = blockIdx.x;
    // The block index for the batch.
    const int bidb = Split ? blockIdx.z / params.h : blockIdx.y;
    // The block index for the head.
    ...
    const int n_split_idx = Split ? blockIdx.y : 0;
    const int num_n_splits = Split ? gridDim.y : 1;
    // Call the direct pointer table implementation
    FLASH_NAMESPACE::flexi_direct_compute_attn_1rowblock_splitkv<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV>(params, bidb, bidh, m_block, n_split_idx, num_n_splits);
}
